import { describeError, rpcCall } from "../api.js";
...
export function buildProfileToolsSection({ signal, initialProfile = null, onProfileChanged } = {}) {
    ...
    const toolGroups = h("div", { class: "settings-tool-groups", "aria-live": "polite" }, h("p", { class: "settings-hint" }, "Loading tool access…"));
    const status = h("p", { class: "settings-status", role: "status", "aria-live": "polite" });
    const resetButton = h("button", { type: "button", class: "btn btn--ghost", disabled: "disabled" }, "Restore defaults");
    const saveButton = h("button", { type: "button", class: "btn btn--primary", disabled: "disabled" }, "Save tool access");
    const element = h("section", { class: "settings-section" }, h("h2", { class: "settings-section-title" }, "Tool access"), h("p", { class: "settings-hint settings-section-intro" }, "Choose exactly which tools each personality can use."), h("label", { class: "settings-field" }, h("span", { class: "settings-label" }, "Configure for"), profileSelect), summary, toolGroups, h("div", { class: "settings-actions settings-toolset-actions" }, resetButton, saveButton), status);
    let currentPayload = null;
    let initialEnabledTools = new Set();
    let dirty = false;
    let busy = false;
    function selectedToolIds() {
        return Array.from(toolGroups.querySelectorAll('input[type="checkbox"]:checked')).map((checkbox) => checkbox.value);
    }
    function selectionChanged() {
        const selectedTools = selectedToolIds();
        return (selectedTools.length !== initialEnabledTools.size ||
    ...
    function populateProfiles(payload) {
        ...
        for (const profile of payload.profiles) {
            ...
            profileSelect.appendChild(h("option", { value: profile.id, selected: profile.id === payload.profile ? "selected" : null }, label));
        }
    }
    function renderSummary(payload) {
        ...
        if (payload.is_active)
            pills.push(h("span", { class: "settings-pill is-active" }, "Active"));
        if (!payload.editable)
            pills.push(h("span", { class: "settings-pill" }, "Locked"));
        summary.replaceChildren(h("div", { class: "settings-toolset-summary-copy" }, h("strong", null, `${enabledCount} ${enabledCount === 1 ? "tool" : "tools"} enabled`), h("span", { class: "settings-toolset-mode" }, payload.overridden ? "Customized for this personality" : "Using profile defaults")), ...pills);
    }
    function renderToolGroups(payload) {
        ...
        for (const group of groups) {
            const fieldset = h("fieldset", { class: "settings-tool-group" }, h("legend", { class: "settings-tool-group-title" }, group.title, h("span", { class: "settings-tool-group-count" }, String(group.tools.length))), h("div", { class: "settings-tool-grid" }, ...group.tools.map((tool) => toolChoice(tool, enabled.has(tool.id), group.unavailable))));
    ...
    saveButton.addEventListener("click", async () => {
        if (saveButton.disabled || !currentPayload)
            return;
    ...
    resetButton.addEventListener("click", async () => {
        if (resetButton.disabled || !currentPayload)
            return;
...
function toolChoice(tool, checked, unavailable) {
    const input = h("input", {
        type: "checkbox",
        value: tool.id,
    ...
    }, input, h("span", { class: "settings-tool-choice-copy" }, h("strong", { class: "settings-tool-choice-name" }, prettifyToolName(tool.id)), unavailable ? h("code", { class: "settings-tool-choice-id" }, tool.id) : null, tool.description ? h("span", { class: "settings-tool-choice-description" }, tool.description) : null));
}
