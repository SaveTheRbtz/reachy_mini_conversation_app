...
export async function mountToolsView({ outlet, signal, searchParams, setLeaveGuard, replaceRoute }) {
    ...
    const profileToolsSection = buildProfileToolsSection({
        ...
        onProfileChanged(profile) {
            ...
            if (fromPersonalities)
                params.set("from", "personalities");
    ...
    const view = h("section", { class: "view view--tools" }, h("header", { class: "view-header" }, h("h1", { class: "view-title" }, "Tools"), h("p", { class: "view-subtitle" }, "Choose which tools each personality can use.")), profileToolsSection.element);
