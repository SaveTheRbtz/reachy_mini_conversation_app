...
export async function mountHomeView({ outlet, signal, navigate }) {
    ...
    const view = h("section", { class: "view view--home" }, h("header", { class: "view-header" }, h("h1", { class: "view-title" }, "Choose a personality"), h("p", { class: "view-subtitle" }, "Pick how Reachy Mini should think and talk. Tap a card to start a conversation.")), grid);
    ...
    try {
        ...
        if (signal.aborted)
            return;
        grid.replaceChildren(h("div", { class: "view-error" }, h("p", null, "Could not list personalities"), h("p", { class: "muted small" }, describeError(error))));
        return;
    }
    if (signal.aborted)
        return;
    ...
    for (const name of choices) {
        ...
        grid.appendChild(buildPersonalityCard({
            name,
            isActive: name === current,
            disabled,
            onSelect: () => handleSelection(name),
            onManageTools: () => navigate(`${ROUTES.TOOLS}?profile=${encodeURIComponent(name)}&from=personalities`),
...
function buildPersonalityCard({ name, isActive, disabled, onSelect, onManageTools, onEdit, onDelete, }) {
    ...
    }, h("span", { class: "personality-card__avatar" }, h("img", {
        src: avatarFor(stripUserPrefix(name)),
        alt: "",
        loading: "lazy",
        "aria-hidden": "true",
        class: !hasAvatar ? "personality-card__avatar--fallback" : null,
    })), h("span", { class: "personality-card__name" }, prettifyProfileName(name)), isActive && checkBadge());
    const slot = h("div", {
    ...
    if (onDelete)
        slot.appendChild(buildDeleteButton({ name, onDelete: () => onDelete(slot) }));
    if (onEdit)
        slot.appendChild(buildEditButton({ name, onEdit }));
    return slot;
}
...
function buildCustomCard({ onClick }) {
    const card = h("button", {
        ...
        onClick,
    }, h("span", { class: "personality-card__plus", "aria-hidden": "true" }, "+"), h("span", { class: "personality-card__name" }, "Custom"), h("span", { class: "personality-card__hint" }, "Write your own prompt"));
