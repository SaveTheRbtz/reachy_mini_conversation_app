...
import { h } from "./ui.ts";
import { GLOW_BY_STATE, ORB_STATES, type OrbState } from "./constants.ts";

/** Build the microphone control and its independent playback animation. */
export function createOrb({ initialState = ORB_STATES.IDLE }: { initialState?: OrbState } = {}) {
  const indicator = h(
    "span",
  ...
  function setState(nextState: OrbState, playing = false) {
